...
resource "aws_security_group" "alb" {
  ...
  description = "ALB - allow 443 from internet, 80 for redirect only"
  vpc_id      = var.vpc_id
...
resource "aws_lb_target_group" "dashboard" {
  ...
  tags = { Name = "${var.project}-${var.environment}-dashboard" }
}
...
resource "aws_lb_listener_rule" "api" {
  ...
  condition {
    path_pattern {
      values = ["/shorten*", "/r/*", "/urls*", "/health*", "/metrics*"]
    }
  }
}
...
resource "aws_lb_listener_rule" "dashboard" {
  ...
  condition {
    path_pattern {
      values = ["/top-urls*", "/hourly*", "/recent*", "/summary*"]
    }
  }
}
